...
describe('ADD_TASK', () => {
  ...
  it('Add one task to populated state', () => {
    const name = "test task";
    const reduced = reducer(mockState, { type: 'ADD_TASK', payload: { id: uuid(), name } });
...
describe('REMOVE_TASK', () => {
  it('Handle remove for nonexistent id on empty state', () => {
    const reduced = reducer([], { type: 'REMOVE_TASK', payload: { id: uuid() } });
    
    expect(reduced).toEqual([]);
  });

  it('Handle remove for nonexistent id on populated state', () => {
    const reduced = reducer(mockState, { type: 'REMOVE_TASK', payload: { id: uuid() } });
  ...
  it('Remove task', () => {
    ...
    const reduced = reducer(mockState, { type: ' REMOVE_TASK', payload: { id: idToRemove } });
